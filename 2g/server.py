...
t_ROUTES = [
	...
	("/reboot","/www/reboot.html"),
	("/reset","/www/reset.html"),
...
class c_HANDLER(SimpleHTTPRequestHandler):
	...
	def do_GET(self):
		try:
			...
			if s_PATH.endswith("/shutdown.html"):
				...
				global b_RUNNING
				b_RUNNING = False
				f = open(MAIN_DIR + "/CMD_POWER","w")
				f.write("shutdown")
				f.close()
			elif s_PATH.endswith("/reboot.html"):
				...
				f.write("reboot")
				f.close()
			elif s_PATH.endswith("/reset.html"):
				print("Request to reset")
				global b_RUNNING
				f = open(MAIN_DIR + "/CMD_RESET","w")
				f.write("reset")
				f.close()
	...
	def do_POST(self):
		...
		if s_PATH.endswith("/config.html"):
			...
			global file_CFG
			file_CFG = open(MAIN_DIR + "/cfg/config","w")
			# NETMODE
			...
			if "fm_dns" in VARIABLES:
				...
				if VARIABLES["fm_dns_entries"][0]:
					...
					with open(MAIN_DIR + "/cfg/hosts.tmp","r") as src:
						with open(MAIN_DIR + "/cfg/hosts","a") as dest:
							for line in src:
